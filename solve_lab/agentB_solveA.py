#!/usr/bin/env python3
"""Start from best_agentA_39021 (core solved: loads=0), set wire=1 to activate handles, then heal
the ripple(12)+unpacking(13) via iterated global linear solve over (handles + movable wire), with
the core wire members fixed. Report failing; if mod-p solved, save state for Dixon lift."""
...
def report(env, tag):
    ...
    fu = sorted(set(res)&UNPACK); fc = sorted(set(res)&CORE); fo = sorted(set(res)-UNPACK-CORE)
    print(f"[{tag}] fail={len(res)}: core={len(fc)}, unpack={len(fu)}, other={len(fo)} {fo[:14]}")
    return res

# wire=p baseline (reproduce agentA)
env.set_free(freevals)
report(env, "wire=p base")

# wire=1
env.forced = {v: (s % p) for v, s in wire.items()}
env.jac_free = wireset - CORE16
env.set_free(freevals)
report(env, "wire=1")
...
for it in range(30):
    ...
    if not res2:
        ...
        print("  saved mod-p solution (agentA base)"); break
    if incons and it >= 1:
