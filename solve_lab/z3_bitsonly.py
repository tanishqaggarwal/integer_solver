...
def main():
    ...
    # FIX free value-inputs to 0; only bits are free
    freeset=set(free_vi)
    for b in mainbits:
    ...
    for poly in atoms:
        ...
        for m, c in poly:
            term = z3.IntVal(c)
            for var in m:
                term = term * (z3.IntVal(0) if var in freeset else (zv(var) if var in mainv else z3.IntVal(fixed[var])))
            expr = expr + term
        s.add(expr == 0); ncon += 1
    ...
    if r == z3.sat:
        ...
        json.dump(out, open('cand_z3bitsonly.json', 'w'))
        print("*** SAT! wrote cand_z3bitsonly.json — verify over Z ***", flush=True)
