...
def feas(pat):
    ...
    fwd(); setf(7068,H.val[2099]%p); setf(4432,H.val[19964]%p)
    b0=resid(); Jt=[]
    bc={v:H.val[v] for v in mov}
    for v in mov:
        o=bc[v]; setf(v,o+1); r1=resid(); setf(v,o)
        Jt.append([(r1[k]-b0[k])%p for k in range(m)])
    MJ=flint.fmpz_mod_mat(m,n,ctx); Mb=flint.fmpz_mod_mat(m,n+1,ctx)
    for k in range(m):
        for j in range(n): MJ[k,j]=Jt[j][k]; Mb[k,j]=Jt[j][k]
        Mb[k,n]=(-b0[k])%p
    return MJ.rank()==Mb.rank()
feasible=[pat for pat in itertools.product([0,1],repeat=10) if feas(pat)]
print(f"feasible: {len(feasible)}")
# per-bit forcing
for i,b in enumerate(bits10):
    vals=set(pat[i] for pat in feasible)
    print(f"  bit x_{b}: {'FORCED='+str(vals.pop()) if len(vals)==1 else 'FREE (both)'}")
# does any feasible have 2081=1?
f2081=[pat for pat in feasible if pat[0]==1]
print(f"feasible patterns with x_2081=1: {len(f2081)}")
if f2081:
    print("  example:",f2081[0],[bits10[j] for j in range(10) if f2081[0][j]])
