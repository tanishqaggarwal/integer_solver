#!/usr/bin/env python3
"""Constrained accumulating heal: keep x_3558,x_29322 fixed (2 constraint rows) while ALL handles
(incl x_24908 cone) heal the accumulated broken wiring. Dixon integer solve. Re-set quotients."""
...
def resid_row(gatevar, H):
    # jacobian of gate value wrt handles (for constraint d(gate)=0)
    base=val[gatevar]; row=[]
    for h in H:
        o=val[h]; val[h]=o+1; forward(); row.append(val[gatevar]-base); val[h]=o
    forward(); return row
Faccum=set()
for rnd in range(12):
    val[30317]=-(val[11150]//p); val[2936]=(537773*val[37758])//p; val[5146]=val[25739]//(6672769*p); forward(); ns['v']=val
    ...
    Faccum|=set(Fnc); Fl=sorted(Faccum)
    H=sorted((set().union(*[eqvars[i]&freeinp for i in Fl]) | xcone) - {30317,2936,5146})
    print(f"round {rnd}: {len(Fnc)} broken now, {len(Fl)} accum, {len(H)} handles", flush=True)
    ns['v']=val
    base=[eval(rootcode_of(i),ns) for i in Fl]
    m=len(Fl); nH=len(H)
    Jac=[[0]*nH for _ in range(m+2)]
    b3=val[3558]; b29=val[29322]
    for j,h in enumerate(H):
        o=val[h]; val[h]=o+1; forward(); ns['v']=val
        for ri in range(m): Jac[ri][j]=eval(rootcode_of(Fl[ri]),ns)-base[ri]
        Jac[m][j]=val[3558]-b3; Jac[m+1][j]=val[29322]-b29
        val[h]=o
    forward(); ns['v']=val
    rhs_full=[-base[i] for i in range(m)]+[0,0]
    # pivot to full-rank square subsystem
    pr,pc=gfp_pivots(Jac); r=len(pr)
    M=[[Jac[pr[i]][pc[j]] for j in range(r)] for i in range(r)]
    rhsv=[rhs_full[pr[i]] for i in range(r)]
    y=dixon(M,rhsv)
    if y is None: print(f"  dixon failed rank {r}"); break
    for j in range(r): val[H[pc[j]]]+=y[j]
    forward(); ns['v']=val
    print(f"  rank {r}/{m+2}; S%p={val[35389]%p==0}, T%p={val[6671]%p==0}, x3558%p={val[3558]%p==0}", flush=True)
val[30317]=-(val[11150]//p); val[2936]=(537773*val[37758])//p; val[5146]=val[25739]//(6672769*p); forward(); ns['v']=val
...
print(f"non-core fail ({len([i for i in F if i not in CORE])}): {sorted(i for i in F if i not in CORE)[:40]}")
print(f"S%p={val[35389]%p}, T%p={val[6671]%p}, L2/p%6672769={(val[25739]//p)%6672769}")
if all(i in CORE for i in F):
    json.dump({f"x_{i}":val[i] for i in range(NVARS)}, open('heal4_state.json','w')); print("SAVED heal4_state.json")
