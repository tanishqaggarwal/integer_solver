...
def apply(sol, tag):
    ...
    if len(ff) < 7:
        json.dump({"x_%d" % j: int(v[j]) for j in range(E.NV) if v[j] != 0},
                  open('S_basin_%d.json' % (39033 - len(ff)), 'w'))
        json.dump({str(x): str(int(y)) for x, y in ns.items()}, open('S_basin_seed.json', 'w'))
        print("  *** WROTE S_basin_%d.json ***" % (39033 - len(ff)), flush=True)
    return len(ff)
...
if sol is not None:
    ...
    best = 10 ** 9
    for k in (1, 2, 3):
        for S in itertools.combinations(cand[:12], k):
