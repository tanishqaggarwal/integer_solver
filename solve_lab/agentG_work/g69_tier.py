"""TIER: EXHAUSTIVE minimum-cost departure on a support: group the affine rows into distinct
...
def cubic_ok(K):
    ...
    return None,len(eff)
# --- report the enumeration size BEFORE running, so exhaustiveness is auditable ---
mults=sorted(len(v) for _,v in dirs)
print('multiplicity histogram: %s'%dict(collections.Counter(mults)))
small=[j for j in range(len(dirs)) if len(dirs[j][1])<=BUDGET]
print('directions with multiplicity <= budget %d : %d'%(BUDGET,len(small)))
def cnt(start,tot):
    c=0
    ...
    return c
NC=cnt(0,0)
print('candidate violated-sets with total multiplicity <= %d : %d  (enumeration is EXHAUSTIVE)'%(BUDGET,NC),flush=True)
if NC>4*10**6:
    print('*** ENUMERATION TOO LARGE TO BE EXHAUSTIVE AT THIS BUDGET — STOPPING ***'); sys.exit(0)
t0=time.time(); found=[]
for w in range(1,BUDGET+1):
    ...
    tested=[0]
    rec(0,[],0)
    print('  budget %d: %d admissible relaxations tested'%(w,tested[0]),flush=True)
    if found:
