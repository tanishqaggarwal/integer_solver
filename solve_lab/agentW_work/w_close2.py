"""W round 2, item 1: THE CLOSING TEST on the CORRECTED knob set K+ = 40.
Same lemma as round 1, restated for 40 knobs and 198 homogeneous SAT rows.
...
breaks.  SAT rows are homogeneous, so x lies in ker_Z(A_SAT\T) = Z^40 cap ker_Q(A_SAT\T).
...
cc = [frozenset(int(x) if x != 'S' else 'S' for x in T) for T in json.load(open('w_cocirc2_raw.json'))['supports']]
...
for k in range(1, 8):
    for P in itertools.combinations(S.FAIL, k):
        ...
        if gain and gain > best[0]:
            ...
            S.price(sol, 'buy %s break %s' % (list(P), mbB), tagfile="close2")
...
json.dump({'nminimal': len(minimal), 'nBs': len(Bs), 'res': res}, open('w_close2.json', 'w'), indent=1)
