...
forward(); ns['v']=val
# free-input backward cone for deep handles
gdmap={t:vids for t,rhs,vids in gates}
...
def freecone(root):
    ...
    while st:
        ...
        seen.add(x)
        if x in gdmap:
            for u in gdmap[x]: st.append(u)
        elif x in freeinp: lv.add(x)
    _fc[root]=lv; return lv
...
for rnd in range(20):
    ...
    for i in F:
        for v in eqvars[i]:
            if v in freeinp: Hset.add(v)
            else: Hset|=freecone(v)
    H=sorted(Hset-set(wire))
