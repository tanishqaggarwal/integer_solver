#!/usr/bin/env python3
"""METHODOLOGY ESCAPE: pin the 220-var identity wire to sign*1 (NOT the forced p). Then the core
product-slacks become coeff-1 (x_4007=1*x_30317 etc.) -> M1,M3 trivially solvable, M2 needs 6672769|L2.
Greedy-construct everything (re-fixing the ~13 unpackings that used wire=p). Verify."""
...
# signed union-find for the wire
...
# standard single-var pins EXCEPT the wire (we pin wire to sign*WIREV)
for pp in A:
    vs=atom_vars(pp)
    if len(vs)==1:
...
for it in range(400):
    ...
    if it%20==0 or len(F)<40: print(f"iter {it}: {len(lines)-len(F)}/{len(lines)} ({len(F)} fail); wire={WIREV}", flush=True)
...
if len(F)==0:
    ...
    json.dump({f"x_{i}":val[i] for i in range(NVARS)}, open('wire1_partial.json','w')); print("saved wire1_partial.json")
