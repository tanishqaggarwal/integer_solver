#!/usr/bin/env python3
"""Forward-construct but ZERO the two control differences x_29322=x_14853-x_12186 and
x_3558=x_24908-x_16742 (both have a free-input side) -> S=x_35389 and T=x_6671 become 0 exactly
-> all three loads L1,L2,L3 = 0 -> quadratic core M1=M2=M3=0 with quotient handles at 0.
Derived pins x_14853:=x_12186, x_16742:=x_24908 refreshed each pass; greedy heals the wiring."""
...
# quotient handles start at 0; control freeinputs handled specially
...
def forward():
    ...
    # apply derived control pins AFTER forward so they track x_24908; then re-forward downstream
def apply_pins():
    val[14853]=val[12186]      # -> x_29322 = 0
    # x_24908 must be computed; forward already did. set partner:
    val[16742]=val[24908]%p    # -> x_3558 = p*floor(x_24908/p), ==0 mod p, small
def evn(node):
...
# control free inputs are determined from the start (never set greedily)
CTRL={14853,16742,12186, 30317,5146,2936}
determined=set(v for v in freeinp if pinned[v])|CTRL
...
def try_set(t):
    ...
    return False
forward(); apply_pins(); forward()
for it in range(400):
    apply_pins(); forward(); ns['v']=val
    F=[i for i in range(len(lines)) if eval(eqcode[i],ns)!=0]
    if it%25==0 or len(F)<40: print(f"iter {it}: {len(lines)-len(F)}/{len(lines)} ({len(F)} fail); det={len(determined)}", flush=True)
...
apply_pins(); forward()
# set quotient handles to zero M1,M3 (M2 handled separately)
val[30317]=-(val[11150]//p); val[2936]=(537773*val[37758])//p; val[5146]=val[25739]//(6672769*p)
apply_pins(); forward(); ns['v']=val
F=[i for i in range(len(lines)) if eval(eqcode[i],ns)!=0]
print(f"L2/p%6672769={(val[25739]//p)%6672769}, S%p={val[35389]%p}, T%p={val[6671]%p}")
print(f"FINAL: {len(lines)-len(F)}/{len(lines)} ({len(F)} fail): {F[:25]}", flush=True)
print(f"S=x_35389={val[35389]}, T=x_6671={val[6671]}")
if len(F)==0:
    json.dump({f"x_{i}":val[i] for i in range(NVARS)}, open('fc_zero_solved.json','w')); print("SAVED fc_zero_solved.json")
elif len(F)<20:
    json.dump({f"x_{i}":val[i] for i in range(NVARS)}, open('fc_zero_partial.json','w')); print("saved partial")
