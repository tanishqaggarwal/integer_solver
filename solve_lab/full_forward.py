#!/usr/bin/env python3
"""Full forward re-evaluation with product-priority matching.

Seed free inputs (unmatched vars) from best; recompute every derived var from its
gate, sweeping in eval order until fixpoint. With product/square gates oriented
forward (the true circuit orientation) this should reproduce a globally
consistent straight-line evaluation if best's free inputs are correct."""
...
def build_matching(atoms, avars):
    ...
    for v in range(NVARS):
        if match[v] is None and cand_atoms[v]:
            augment(v, set())
    return match, has_prod

def main():
    t0 = time.time()
    atoms = load_atoms(); avars = [atom_vars(p) for p in atoms]
    match, has_prod = build_matching(atoms, avars)
    derived = [v for v in range(NVARS) if match[v] is not None]
    nprod = sum(1 for v in derived if has_prod[match[v]])
    print(f"derived {len(derived)} ({nprod} product-defined), free {NVARS-len(derived)} ({time.time()-t0:.0f}s)", flush=True)
    ...
    json.dump({f"x_{i}": val[i] for i in range(NVARS)}, open('cand_fullfwd.json', 'w'))
    print("wrote cand_fullfwd.json", flush=True)
