...
def fcone(root):
    ...
    return lv
extra|=fcone(24908)
# full closure over shared handles starting from core + control handles
...
Hc=set(W0)|extra|{14853,12186,16742}
for _ in range(30):
    eqs2=set()
    for h in Hc: eqs2|=byh[h]
    newH=set()
    for i in eqs2: newH|=(eqvars[i]&freeinp)
    if newH<=Hc: break
    Hc|=newH
W=sorted(Hc-set(override))
print(f"closure witness inputs: {len(W)}", flush=True)
