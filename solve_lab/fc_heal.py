#!/usr/bin/env python3
"""From best (39013): minimal residue fix -> S=T=0 mod p; set quotient handles (M1,M3=0);
pin control set {14853,12186,16742,19750}+quotients so S,T residues stay frozen; greedy-heal the
23 broken wiring eqs with the other handles. Report core status; M2 handled separately."""
...
# quotient handles (M1,M3 exact; M2 floored for now)
val[30317]=-(val[11150]//p)
val[2936]=(537773*val[37758])//p
val[5146]=val[25739]//(6672769*p)
forward()
...
for it in range(300):
    ...
    if it%20==0 or len(F)<40: print(f"iter {it}: {len(lines)-len(F)}/{len(lines)} ({len(F)} fail); core_fail={len([i for i in F if i in CORE])}", flush=True)
    if not F: print("SOLVED!"); break
    changed=False
    for i in F:
        if i in CORE: continue  # don't touch core with greedy (quadratic)
...
ns['v']=val
F=[i for i in range(len(lines)) if eval(eqcode[i],ns)!=0]
print(f"FINAL: {len(lines)-len(F)}/{len(lines)} ({len(F)} fail)")
print(f"core fail: {sorted(i for i in F if i in CORE)}")
...
if len([i for i in F if i not in CORE])==0:
    json.dump({f"x_{i}":val[i] for i in range(NVARS)}, open('fc_heal_state.json','w')); print("saved fc_heal_state.json (wiring healed)")
