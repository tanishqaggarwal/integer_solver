...
for s in ST:
    ...
    val[s['u3']]=random.randrange(p); val[s['y3']]=random.randrange(p)   # GARBAGE output
    ev(o1,val); tested+=1
    if val[s['R1']]%p==0: ok+=1
    else: nz+=1
print('gadgets fed two EQUAL live points, output set to a RANDOM (wrong) value:')
print('  residual R1 STILL vanishes (constraint is vacuous here): %d / %d'%(ok,tested))
print('  residual R1 nonzero  (output genuinely constrained): %d / %d'%(nz,tested))
