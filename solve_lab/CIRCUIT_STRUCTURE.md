...
- Each gadget is **fraction-free**, over register pairs (w1,w2),(w3,w4) -> (w5,w6):
  ```
  A = (w5+w1+w3+a2)*(w3-w1)^2 - (w4-w2)^2   == 0
  B = (w6+w2)*(w3-w1) - (w4-w2)*(w1-w5)     == 0
  ```
  gated by `sel = b1*b2` through 3 random combinations `sel*(c1*A + c2*B) = p*(free)`.
...
1. **Division wires — weak form confirmed.** All 383 lambda-wires use
   `w*(w3-w1) = (w4-w2)`; there are ZERO wires of the strong form `w*(w3-w1) = 1`.
...
4. **No variety-style consistency check exists** — inconsistent intermediate registers pass
