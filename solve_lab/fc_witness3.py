...
def fcone(root):
    ...
    return lv
extra|=fcone(24908)
# 2-hop: add free inputs of all equations containing any current extra/control handle
byh2=defaultdict(set)
for _i in range(len(lines)):
    for _v in eqvars[_i]&freeinp: byh2[_v].add(_i)
hop1eqs=set()
for h in (extra|{14853,12186,16742}):
    hop1eqs|=byh2[h]
extra2=set()
for i in hop1eqs: extra2|=(eqvars[i]&freeinp)
W=sorted((W0|extra|extra2)-set(override))
print(f"expanded 2-hop witness inputs: {len(W)} (base {len(W0)})", flush=True)
