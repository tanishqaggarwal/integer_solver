...
def build(selset):
    ...
    obstruct=[]; setzero=0
    ORD={a:k for k,a in enumerate(topo)}
    q=[(ORD.get(i,10**9),i) for i in range(len(AP))]; heapq.heapify(q)
    inq=[True]*len(AP)
    def push(i):
        if not inq[i]:
            inq[i]=True; heapq.heappush(q,(ORD.get(i,10**9),i))
    while q:
        _,i=heapq.heappop(q); inq[i]=False; ap=AP[i]
        unk=sorted({x for m in ap for x in m if val[x] is None})
        if len(unk)==2:
            hs=[y for y in unk if y in HANDLE and HDEF[y]!=i]
            if len(hs)==1:
                val[hs[0]]=0; setzero+=1
                for j2 in v2a[hs[0]]: push(j2)
                push(i)
            continue
        ...
        if sq:
            ...
            if (-Bc)%A!=0: obstruct.append((i,y,A,Bc,ORD.get(i,-1)))
            val[y]=(-Bc)//A
        for j2 in v2a[y]: push(j2)
