...
| **O's seven-way 1-for-1 trade** | over its **34 inputs** | O; **reported, T did not re-run** | mechanism is O's Lemma, which T verified (§1) |
| **N's detach exhaustion** | under **`fwd2`'s orientation** | N; **reported, T did not re-run** | a different orientation convention |
...
| **L's `\|S\|=2` closure generalises** | **NOT established.** One ON-set. `\|S\|=17` still ends 1 undischarged; the obstruction is shared-wire simultaneity (the per-condition guard verifies only its target atom), not the fit or the degree bound | — | `\|S\| = 3, 5, 8` — dumped and checker-run |
