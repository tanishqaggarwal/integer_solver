...
order = sorted(rows, key=lambda c: (-cost1(c), c))
...
print(f'=> sacrificing them would score at most {L.NEQ - len(eqs)}')
json.dump({'wit': wit, 'cost': len(eqs)}, open(os.path.join(HERE, 'wit4.json'), 'w'))
