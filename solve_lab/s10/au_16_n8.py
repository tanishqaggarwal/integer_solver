...
K=(v[4432]-v[19964])%P
print('C0',C0); print('K ',K); print('A1_0',A[1]%P)
ATOMS=SEVEN+[22231]
rows=[]
for e in E12:
    m,sq,co=L.eq_atoms[e]; rows.append([co.get(a,0) for a in ATOMS])
for e,r in zip(E12,rows): print(f'  eq {e:>6}: {r}')
def kernel_Q(M,n):
...
def feasible(K_basis, conds):
    """conds = list of (coeff-vector over atoms, rhs). solve for lambdas mod p."""
    m=len(K_basis)
    M=[]
    for cvec,rhs in conds:
        M.append([sum(cvec[i]*wv[i] for i in range(len(wv)))%P for wv in K_basis]+[rhs%P])
    nr=len(M); r_=0
    for j in range(m):
...
# congruences over (A0..A6,B): c1: A0 + 7376877*A6 == C0 ; c2: A1 + B == K
c1=[1,0,0,0,0,0,7376877,0]
c2=[0,1,0,0,0,0,0,1]
print('\n=== n=8 (seven + a22231): subsets of the 12 by size ===')
for k in range(12,3,-1):
    dist=collections.Counter(); good=[]
    for sel in itertools.combinations(range(12),k):
        Kb=kernel_Q([rows[i] for i in sel],8)
        dist[len(Kb)]+=1
        if not Kb: continue
        if feasible(Kb,[(c1,C0),(c2,K)]): good.append((sel,Kb))
    print(f' size {k}: kerdim dist {dict(sorted(dist.items()))}  congruence-feasible: {len(good)}')
    if good:
        print('   FIRST FEASIBLE:', [E12[i] for i in good[0][0]], 'kdim',len(good[0][1]))
        print(f'   => {12-k} of the twelve fail (+1 for a37887 if B!=0) ; total {12-k+1}')
        break
