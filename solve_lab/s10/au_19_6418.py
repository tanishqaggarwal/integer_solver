...
for u in (6418, 12553):
    print(f'--- x_{u} appears in atoms: ---')
    for a in sorted(L.var_atoms[u]):
        print(f'   a{a:<6} out={L.atom_out.get(a)} neq={len(L.atom2eq.get(a,{}))} src={L.atom_src[a][:100]}')
    print()
for u,d in ((6418,1),(12553,1)):
    ...
    print(f'x_{u}+{d}: score={L.NEQ-len(f)} out12={len(f-E)} nonzero atoms outside seven: {nzout}')
    for a in nzout:
        print(f'    a{a}: neq={len(L.atom2eq.get(a,{}))} out={L.atom_out.get(a)} src={L.atom_src[a][:120]}')
        print(f'        vars={sorted(L.avars[a])}, free among them={[x for x in sorted(L.avars[a]) if x not in definer]}')
