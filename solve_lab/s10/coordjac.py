"""S11 step 98: the exact map from EVERY free input to the six coordinates.

Part XXVII said all seven quantities of the addition are literal constants.  That was
too strong, and the support computation says why:

    x1 = x12186   computed, 417-input support        y1 = x16742   FREE
    x2 = x14853   FREE                               y2 = x24908   computed, 99 inputs
    x3 = x22162   FREE                               y3 = x30213   FREE
    K  = x24453   constant (empty support)           <- only this one is really fixed

and perturbing x22152, x33462, x6418 or x12553 moves NONE of x1, y1, x2, y2.  Those
four literals are separate variables that merely happen to carry the same residues at
the current state; they do not feed the addition.  So the coordinates are constrained
by congruences to COMPUTED targets, not pinned to constants, and the question becomes
what actually moves them.

One forward-AD pass per free input gives the exact derivative of every coordinate --
7,273 passes, affordable -- so the linear map from the whole free-input space to the
six-dimensional coordinate space can be written down exactly.

Usage: coordjac.py START END [state.json]
"""
...
NAMES = dict(zip(KEY, ['x1', 'y1', 'x2', 'y2', 'x3', 'y3', 'y1_tgt', 'x2_tgt',
                       'A', 'B']))
...
print('free inputs that move each coordinate:')
for nm in ['x1', 'y1', 'x2', 'y2', 'x3', 'y3', 'y1_tgt', 'x2_tgt', 'A', 'B']:
    print('   %-7s %-5d   sample %s' % (nm, hits[nm], movers[nm][:10]))
json.dump({k: v2 for k, v2 in movers.items()},
          open(os.path.join(HERE, 'coordmovers.json'), 'w'))
print('saved coordmovers.json')
