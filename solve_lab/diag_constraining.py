#!/usr/bin/env python3
"""Count genuinely-constraining equations: with free inputs at best but ONE load-cone perturbed,
how many fail? Better: count equations whose residual is affected by SOME free input (non-definition).
Simplest proxy: how many equations does forward_construct's determined-set touch."""
...
# set free inputs to distinct pseudo-random values (deterministic), forward, count holds
for idx,v in enumerate(sorted(freeinp)):
    if not pinned[v]: val[v]=(1234567*(idx+1)+89) % p
forward(); ns['v']=val
holds=sum(1 for i in range(len(lines)) if eval(eqcode[i],ns)==0)
print(f"with random free inputs: {holds}/{len(lines)} auto-hold (definitions), {len(lines)-holds} constraining")
