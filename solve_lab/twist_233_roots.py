...
from propagate import atom_vars, NVARS, load_atoms
...
def main():
    ...
    # replace ALL deg-4 perfect squares with their deg-2 roots (Q^2=0 <=> Q=0)
    nrepl=0
    for a in range(len(A)):
        p=A[a]
        if p and max(len(m) for m in p)==4:
            Q=try_sqrt(p)
            if Q: A[a]=Q; nrepl+=1
    print(f"replaced {nrepl} deg-4 squares with roots", flush=True)
