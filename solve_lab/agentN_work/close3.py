"""Bottom-up cascade closer, with fallback to indirect (support) free inputs."""
...
def try_atom(st,a,frozen,pool):
    best=None
    for X in pool:
        if X in frozen: continue
...
def close(st, maxsteps=800, verbose=False, frozen=None, indirect=True):
    st=st.clone(); frozen=set(frozen or ())
    for step in range(maxsteps):
        ...
        moved=False
        for phase in (0,1):
            if phase==1 and not indirect: break
            for a in nz:
                pool=[X for X in AVARS[a] if X in FREE] if phase==0 else [X for X in SUPV[a] if X not in AVARS[a]]
                b=try_atom(st,a,frozen,pool)
                if b is None: continue
                k,g,X,rt=b
                st=g; frozen.add(X); moved=True
                if verbose: print('  s%d p%d a%d(sup%d) <- x_%d : nz=%d fails=%d'%(step,phase,a,SUPN[a],X,k[0],k[1]),flush=True)
                break
            if moved: break
...
if __name__=='__main__':
    ...
    p=os.path.join(HERE,'E_%d_%d_%d.json'%(out.score(),ub,wb))
