...
def main():
    ...
    for i, p in enumerate(eqp):
        ...
        if max((len(m) for m in q), default=0) > 1:
            raise SystemExit("boolean subsystem is not linear")
        if q:
            rows.append((i, q))
    print("boolean-layer linear rows:", len(rows))
    ...
    pickle.dump({'forms': forms, 'pend': list(pend)}, open(os.path.join(HERE, 'zwire.pkl'), 'wb'))
