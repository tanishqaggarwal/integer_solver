...
"""
import pickle, collections, heapq, time, sys, json, os
...
for i, p in enumerate(facs):
    ...
    lin -= quad
    if not quad:
        u = [m[0] for m, c in p.items() if len(m) == 1 and abs(c) == 1 and m[0] in cands[i]]
        if len(u) == 1: pref[i] = u[0]
        continue
...
# reservation: if exactly one gate prefers v as its output, only that gate may define v
_pc = collections.Counter(v for v in pref if v is not None)
reserved = {}
for i, v in enumerate(pref):
    if v is not None and _pc[v] == 1: reserved[v] = i
print("reserved output vars:", len(reserved))

FORBID = set()

def main():
    global FORBID
    if os.path.exists(W+'forbid.pkl'):
        FORBID = set(pickle.load(open(W+'forbid.pkl','rb')))
        print("forbidden promotions:", len(FORBID))
    excl = set()
    ...
    while True:
        while ready:
            ...
            if reserved.get(v, i) != i:
                continue          # v belongs to another gate
            live[i] = False
        ...
        while heap:
            ...
            if v in FORBID:
                skipped.append(heapq.heappop(heap)); continue
            best = v; break
        for it in skipped: heapq.heappush(heap, it)
        if best is None:
            while heap:
                neg, v = heap[0]
                if v in known: heapq.heappop(heap); continue
                ...
                if -neg > cur: heapq.heapreplace(heap, (-cur, v)); continue
                best = v; break
        if best is None and reserved:
            # release reservations that are blocking, then retry
            blocked = set()
            for i in range(NF):
                if not live[i]: continue
                rem = fvars[i] - known
                if len(rem) == 1:
                    v = next(iter(rem))
                    if v in cands[i] and reserved.get(v, i) != i: blocked.add(v)
            if blocked:
                for v in blocked: reserved.pop(v, None)
                for i in range(NF):
                    if live[i] and len(fvars[i] - known) <= 1: ready.append(i)
                continue
        if best is None:
