...
def main():
    ...
    aborted = 0
    mdeg = collections.Counter()
    mdist = collections.Counter()
    with open(EQ) as f:
    ...
    out = {
        ...
        'monomial_degree_hist': dict(sorted(mdeg.items())),
        'monomial_distinctvars_hist': dict(sorted(mdist.items())),
    }
    json.dump(out, open('/home/user/integer_solver/solve_lab/agentAJ_work/mono_summary.json', 'w'), indent=1)
    with open('/home/user/integer_solver/solve_lab/agentAJ_work/mono_rows.tsv', 'w') as g:
    ...
    print('monomial_degree_hist', dict(sorted(mdeg.items())))
    print('monomial_distinctvars_hist', dict(sorted(mdist.items())))
