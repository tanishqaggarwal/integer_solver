...
import sys as _sys
_a7=int(_sys.argv[1]) if len(_sys.argv)>1 else 24601
_a34=int(_sys.argv[2]) if len(_sys.argv)>2 else 2081
override={_a7:1, _a34:1, 30213:C2, 22162:C1, 24468:C1, 18956:C2}
...
for it in range(400):
    ...
    FAILVARS-=determined
    if False: print(f"iter {it}: {len(lines)-len(F)}/{len(lines)} ({len(F)} fail); determined free={len(determined)}", flush=True)
    ...
    forward()
    if not changed: pass; break
ns['v']=val
F=[i for i in range(len(lines)) if eval(eqcode[i],ns)!=0]
print(f"FINAL: {len(lines)-len(F)}/{len(lines)} ({len(F)} fail): {F[:20]}", flush=True)
if len(F)<20:
    json.dump({f"x_{i}":val[i] for i in range(NVARS)}, open(f"fc_{_a7}_{_a34}.json","w")); print("SAVED")
