...
def main():
    ...
    for it in range(40):
        ...
        if r == z3.sat:
            ...
            json.dump(out, open('cand_z3iter2.json', 'w'))
            nz = [v for v in free_vi if out[f"x_{v}"] != 0]
            print(f"*** SAT! nonzero value-inputs: {nz}. wrote cand_z3iter2.json — VERIFY ***", flush=True)
            return
        if r != z3.unsat:
