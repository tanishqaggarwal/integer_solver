...
crit={14853,12186,16742,30317,2936,5146}
ns['v']=val
...
for rnd in range(12):
    # re-pin residues (S,T back to 0 mod p) and re-set quotients each round
    forward(); val[14853]=val[14853]-(val[29322]%p); forward(); val[16742]=val[16742]+(val[3558]%p); forward()
    val[30317]=-(val[11150]//p); val[2936]=(537773*val[37758])//p; val[5146]=val[25739]//(6672769*p); forward()
    ns['v']=val
    ...
    H=sorted(set().union(*[eqvars[i]&freeinp for i in Fnc]) - crit)
    print(f"round {rnd}: {len(Fnc)} broken, {len(H)} handles", flush=True)
