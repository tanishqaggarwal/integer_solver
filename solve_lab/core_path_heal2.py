...
p=H.p
import json as _j
ALLBITS=set(_j.load(open("all_bits.json")))
...
for v in (loadcone_free | ALLBITS | {14853,12186,16742}) & set(range(N)):
    pin(v, V[v]%p)
...
if c2==0:
    ...
    ch=0
    for fv in H.freeinp:
        if fv in loadcone_free or fv in ALLBITS or fv in {14853,12186,16742}: continue
        r,m,o=find2(fv)
        if r==CONST: nr=(m+o)%p
    ...
    json.dump({f'x_{i}':V[i] for i in range(N)},open('core_path_solved2.json','w'))
