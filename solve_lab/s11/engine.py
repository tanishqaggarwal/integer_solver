"""Structural closure map + exact integer Newton on residual checks."""
...
def apply_theta(theta, pin_rounds=25):
    v=[0]*L.NVARS
    for b in BITS: v[b]=1
    for k,x in theta.items(): v[k]=x
    fw.forward(v)
    # structural closure: kill core exactly
    ...
    fw.forward(v)
    locked=set(BITS)|DERIVED|set(theta)
...
if __name__=='__main__':
    t0=time.time()
    theta={5096:0, 19750:0}
    v=apply_theta(theta)
    b=fw.bad_checks(v); av=L.all_atom_values(v); f=L.failing_eqs(av)
    print(f"theta0: bad={len(b)} failing={len(f)} score={L.NEQ-len(f)} ({time.time()-t0:.0f}s)")
    print("bad:", b)
    print("n=%d m=%d a29539res=%d a26731res=%d"%(v[29322],v[3558],
          fw.evalpoly(L.polys[29539],v), fw.evalpoly(L.polys[26731],v)))
