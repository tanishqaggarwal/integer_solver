...
def rep(v,tag):
    ...
    for a in B: print(f"    a{a} in {len(L.atom2eq.get(a,{}))} eqs  free vars {[u for u in sorted(L.avars[a]) if u in FREE]}")
    return B,F
v=load_raw(os.path.join(HERE,'data','modp3_out.json'))
rep(v,'modp3')
for a,t in [(19297,30317),(19299,5146),(30984,2936)]:
    x=lin_solve(a,t,v)
    print(f"  a{a} <- x{t}: {'ok' if x is not None else 'NOT divisible'}")
    if x is not None: L.ripple(v,{t:x})
B,F=rep(v,'after handles')
json.dump({('x_%d'%i):v[i] for i in range(L.NVARS)}, open(os.path.join(HERE,'data','modp4_out.json'),'w'))
