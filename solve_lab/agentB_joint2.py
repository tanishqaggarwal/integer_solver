#!/usr/bin/env python3
"""Wire members are FREE (no hard ties). Pin the 3 core-product members = 1 (keeps core linear),
treat other 217 wire + all handles as free, INCLUDE core rows (linear), iterate joint Newton.
Goal: all 39033 = 0 mod p."""
...
env.jac_free = wireset - COREW                         # core wire pinned at 1
env.set_free({v: best.get(v, 0) for v in env.freeset})
allowed = (wireset - COREW) | handles
...
print(f"[joint2] start: {cats()}")
for it in range(30):
    t = time.time()
    incons, delta = solve(set())
    apply(delta)
    n,u,c,o = cats()
    print(f"  it {it}: fail={n} unpk={u} core={c} oth={o} incons={incons} |d|={len(delta)} {time.time()-t:.1f}s", flush=True)
    if n == 0:
        print("  *** ALL 39033 ROOTS 0 mod p ***")
        pickle.dump({'valp': env.valp[:], 'forced': dict(env.forced)},
                    open('/tmp/claude-0/-home-user-integer-solver/d279c5f7-7ad9-5092-a944-cb7784834 e0/scratchpad/agentB_modp_sol.pkl'.replace(' ',''),'wb'))
        print("  SAVED"); break
    if it >= 4 and n == prev: print("  plateau"); break
    prev = n
