#!/usr/bin/env python3
"""Constructive solve with DOUBLE-WIDTH UNPACKING. For a failing gadget with residual R that has
a coeff-1 (mod p invertible) remainder free input and a coeff-p quotient free input (wire-product),
set both: dr = -R*inv(cr) mod p, then dq = -(R+cr*dr)/cq. Acyclic circuit => topological convergence."""
...
def try_absorb(t):
    ...
    names=sorted(set(int(m.group(1)) for m in re.finditer(r'x_(\d+)', ast.unparse(t))))
    frees=[v for v in names if v in freeinp]
    rem=None; quot=None; linabs=None
    for v in frees:
        c=coeff(t,v)
        if c==0: continue
        if c%p==0 and c!=0:
            if quot is None: quot=(v,c)
        elif abs(c)!=0 and (cur%c==0):
            if linabs is None: linabs=(v,c)
        if c%p!=0 and pow(c%p,p-2,p) is not None:
            if rem is None: rem=(v,c)
    # double-width split: need a remainder (coeff invertible mod p) AND a quotient (coeff mult of p)
    if rem is not None and quot is not None:
        vr,cr=rem; vq,cq=quot
        dr=(-cur*pow(cr%p,p-2,p))%p
        Rp=cur+cr*dr
        if Rp%cq==0:
            val[vr]+=dr; val[vq]+=(-Rp)//cq; return True
    if linabs is not None:
        v,c=linabs; val[v]-=cur//c; return True
    # fallback: remainder alone can't zero (needs quotient); try any free with c|cur
    for v in frees:
        c=coeff(t,v)
        if c!=0 and cur%c==0: val[v]-=cur//c; return True
    return False
forward(); ns['v']=val
for it in range(300):
    ns['v']=val
    F=[i for i in range(len(lines)) if eval(eqcode[i],ns)!=0]
    if it%20==0 or len(F)<30: print(f"iter {it}: {len(lines)-len(F)}/{len(lines)} ({len(F)} fail)", flush=True)
...
if len(F)==0:
    json.dump({f"x_{i}":val[i] for i in range(NVARS)}, open('c2_solved.json','w')); print("SAVED c2_solved.json")
