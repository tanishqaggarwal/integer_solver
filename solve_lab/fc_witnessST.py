...
W=sorted(v for v in freeinp if best.get(v,0)!=0 and v not in override)
print(f"witness slack inputs: {len(W)}", flush=True)
...
for newton in range(12):
    ...
    b3=val[35389]%p; b29=val[6671]%p
    if b3==0 and b29==0: print(f"newton {newton}: S,T residues ZERO"); break
    ...
    for w in W:
        ...
        for i in range(len(lines)):
            ...
            if d: col[i]=d
        cols.append(col)
        dres3.append((val[35389]%p-b3)%p); dres29.append((val[6671]%p-b29)%p)
        val[w]=o
    forward(); ns['v']=val
    ...
    print(f"newton {newton}: applied; S%p={val[35389]%p==0}, T%p={val[6671]%p==0}, noncore_broken={nb}", flush=True)
