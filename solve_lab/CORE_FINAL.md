# The atom-level obstruction — why `EQUATIONS.txt` (new instance) resists the METHODOLOGY escape
...
the exact method that solved the previous instance (see root `METHODOLOGY.md`), and the precise
...
## 2. Both gaps are product slacks (the METHODOLOGY structure is present)
